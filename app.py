...
def get_delete_key(delete_key) -> Response:
...
def upload_shortcuts(enter_key) -> Response:
    if enter_key == ENTER_KEY:
...
def download_by_filename(enter_key, filename) -> Response:
    if enter_key == ENTER_KEY:
        full_path = os.path.join(UPLOAD_FOLDER, filename)
...
def delete_all(admin_key) -> Response:
...
def method_not_allowed(error) -> Tuple[str, int]:
    return render_template("405.html"), 405

@app.errorhandler(413)
def request_entity_too_large(error) -> Tuple[str, int]:
    return render_template("413.html"), 413

@app.errorhandler(429)
def too_many_requests(error) -> Tuple[str, int]:
